...
contract FlashswapTest is Test {
    ...
    function test_ExactOutput_ThreePools() public {
        ...
        Flashswap.ExactOutputParams memory params = Flashswap
            .ExactOutputParams({
                path: _path,
        ...
        assertEq(
            IERC20(WBTC).balanceOf(address(friend)),
            0.5e8,
            "The destination address receives half of the exact amount of output tokens"
        );
        // @notice this test case was orginally checking WETH, it should be WBTC...
        assertEq(
            IERC20(WBTC).balanceOf(address(caller)),
            0.5e8,
            "The caller keeps half of the exact amount of output tokens"
        );
        assertEq(
            caller.data(),
            EXPECTED_DATA,
            "The caller receives the expected data through the callback"
        );
        assertEq(
            caller.amountReceived(),
            _amountOut,
            "The caller receives the expected `amountReceived` through the callback"
        );
    }
    ...
    function test_Friend() public {
        ...
        assertEq(
            IERC20(WETH).balanceOf(address(this)),
            wethAmtOut,
            "The friend gives me the requested amount of WETH"
        );
        assertEq(
            IERC20(WBTC).balanceOf(address(this)),
            0,
            "The friend takes the 0.5e8 WBTC that they want from me"
        );
        assertEq(
            IERC20(WBTC).balanceOf(address(friend)),
            0.5e8,
            "The friend gets the 0.5e8 WBTC that they want"
        );
    }
}
